...
패키지는 flatpak 과 aur/arch 패키지를 사용합니다. 이를 관리하기 위해 metapac 을 사용하며, 패키지 목록은 groups 디랙터리에, metapac 설정은 config.toml 에 존재합니다.
따라서 이 저장소를 ~/.config/metapac 에 클론하면 바로 metapac 싱크를 하여 사용할 수 있습니다.
단 패키지 목록은 qwreey가 사용하는 것을 모두 포함하고, 특정 장치에 제한되는 패키지를 사용하는 등 원하는 바가 다를 수 있으므로, metapac 을 통해 이 저장소가 포함하는 패키지 목록을 동기화 하는것은 바람직하지 않을 수 있습니다.
따라서 패키지 목록을 눈으로 읽어보며 필요한 것이 무엇인지 직접 파악하는것이 좋습니다.
