...
if [ ! -e /boot/grub ]; then
	...
	sudo grub-install --bootloader-id=GRUB --target=x86_64-efi --efi-directory=/boot --removable |& indent
fi

echo "Updating /boot/grub/custom.cfg"
cat <<EOF | sudo tee /boot/grub/custom.cfg | indent
...
echo "Updating /boot/grub/background.png"
...
sudo sed "s|#* *GRUB_CMDLINE_LINUX_DEFAULT=.*|GRUB_CMDLINE_LINUX_DEFAULT=\"$GRUB_KERNEL_CMDLINE\"|g" -i /etc/default/grub
...
sudo grub-mkconfig -o /boot/grub/grub.cfg |& indent
